...
def CalculateAnswer(title):
    # title = str(sys.argv) #this was for running the script from the command line with an argument
    x = [(title, 0)]
    ...
    lb_rl = pickle.load(open(os.path.join(sys.path[0], 'scripts/lb_for_rl.pkl'), 'rb'))
    ...
    cv_for_lda = pickle.load(open(os.path.join(sys.path[0], 'scripts/cv_for_lda.pkl'), 'rb'))
    input_words = cv_for_lda.transform(df_x["proper_title"])

    #transform the input string using the training set model
    lda_8 = pickle.load(open(os.path.join(sys.path[0],'scripts/ttpc_lda.pkl'), 'rb'))
    ...
    tfidf = pickle.load(open(os.path.join(sys.path[0],'scripts/tfidf.pkl'), 'rb'))
    ...
    rfr_final = pickle.load(open(os.path.join(sys.path[0],'scripts/ttpc_rfr.pkl'), 'rb'))
